  ...
  async claimAttendance(playerId: string): Promise<RewardResultDto> {
    ...
    try {
      return await this.dataSource.manager.transaction(async (manager) => {
        const existing = await manager.getRepository(AttendanceLog).findOne({
          where: { playerId, attendanceDate: today },
        ...
        const totalDays = await manager.getRepository(AttendanceLog).count({ where: { playerId } });
      ...
      // Duplicate key from unique constraint race — treat as already claimed
      throw new HttpException(
        { code: ErrorCodes.REWARD_001.code, message: ErrorCodes.REWARD_001.message },
        HttpStatus.CONFLICT,
      );
